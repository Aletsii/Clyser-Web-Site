...
class Reserva {
    ...
    public function guardarReserva( int $idServicio, int $idCliente, int $idProveedor, string $fecha, string $calle, ?int $numero ) : array {
        $sql = "INSERT INTO reserva (idServicio, idCliente, idProveedor, fecha, estadoReserva, calle, numero)
                VALUES (?, ?, ?, ?, 'pendiente', ?, ?)";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("iiissi", $idServicio, $idCliente, $idProveedor, $fecha, $calle, $numero);
...
public function mostrarReservaCliente(int $idCliente) : array{
    $sql = "SELECT
             r.idReserva, r.fecha, r.estadoReserva, r.calle, r.numero,
